...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        # because the output activation function is f(x) = x, it's derivative is 1
        output_error_term = error   
        # Weight step (hidden to output)
        del_w = hidden_outputs * output_error_term
        delta_weights_h_o += del_w[:, None]
        ...
        del_w = np.matmul(X[:, None],  hidden_error_term[:, None].T) 
        # Weight step (input to hidden)
        delta_weights_i_h += del_w
